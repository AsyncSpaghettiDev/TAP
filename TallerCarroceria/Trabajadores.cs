...
namespace TallerCarroceria {
    public partial class Trabajadores : UserControl {
        ...
        private void cargarComboBox( ) {
            ...
            SqlCommand obtenerID = new SqlCommand("SELECT idTrabajador FROM Trabajadores", this.conexion);
            // Se abre la conexión
            this.conexion.Open();
            ...
            while (IDs.Read()) {
                ...
                this.comboBox1.Items.Add(IDs[ "idTrabajador" ].ToString());
            }
            // Se cierra la conexion
            IDs.Close();
            // En caso que haya datos ya cargados se limpian para volver a cargarlos
            ...
            // Se cargan los ID's (campo clave) de la tabla en cuestion
            obtenerID = new SqlCommand("SELECT idEspecialidad FROM Especialidades", this.conexion);
            // Se "guardan" todos los ID's en SqlDataReader, que no es mas que una lista de arrays
            IDs = obtenerID.ExecuteReader();
            // Mientras haya datos en el SqlDataReader se leeran los datos
            while (IDs.Read()) {
                // Se añade ID por ID al comboBox
                this.comboBox2.Items.Add(IDs[ "idEspecialidad" ].ToString());
            }
            // Se cierra la conexion
            this.conexion.Close();
        ...
        public void alta( object sender, EventArgs e ) {
            ...
            SqlCommand altas = new SqlCommand("INSERT INTO Trabajadores VALUES(@ID,@Nombre,@Especialidad)", this.conexion);
            ...
            altas.Parameters.AddWithValue("Especialidad", int.Parse(this.comboBox2.Text));
            // Se abre la conexion
            this.conexion.Open();
            try {
                // Se cargan los datos a la BD
        ...
        public void baja( object sender, EventArgs e ) {
            ...
            SqlCommand bajas = new SqlCommand("DELETE FROM Trabajadores WHERE idTrabajador=@ID", this.conexion);
        ...
        private void actualizar( object sender, EventArgs e ) {
            ...
            SqlCommand actualizar = new SqlCommand("UPDATE Trabajadores SET idTrabajador=@ID,nombreTrabajador=@Nombre,idEspecialidad=@Especialidad WHERE idTrabajador=@ID", this.conexion);
            // Se pasan los valores de los textBox a las variables temporales
            ...
            actualizar.Parameters.AddWithValue("Especialidad", int.Parse(this.comboBox2.Text));
            // Se abre la conexion
            this.conexion.Open();
        ...
        public void buscar( object sender, EventArgs e ) {
            ...
            SqlCommand consulta = new SqlCommand("SELECT * FROM Trabajadores WHERE idTrabajador=@ID", this.conexion);
            ...
            while (data.Read()) {
                ...
                this.comboBox2.SelectedItem = data[ 2 ].ToString();
        ...
        private void comboBox2_DropDownClosed( object sender, EventArgs e ) {
            // Se cargan los ID's (campo clave) de la tabla en cuestion
            SqlCommand obtenerID = new SqlCommand("SELECT nombreEspecialidad FROM Especialidades WHERE idEspecialidad=@ID", this.conexion);
            obtenerID.Parameters.AddWithValue("ID", int.Parse(this.comboBox2.Text));
            // Se abre la conexión
            this.conexion.Open();
            // Se "guardan" todos los ID's en SqlDataReader, que no es mas que una lista de arrays
            SqlDataReader IDs = obtenerID.ExecuteReader();
            if (IDs.Read())
                this.label4.Text = IDs[ 0 ].ToString();
            // Se cierra la conexion
            this.conexion.Close();
        }
    }
